...
#Load data
waning_data_clean <- read.csv("data/ve_waning_predictions_adj_mean.csv")[,-1]
waning_data_clean_95UI <- read.csv("data/ve_waning_predictions_adj_95UI.csv")[,-1]
hosp_death_age_stratified <- read.csv("data/hosp_death_age_stratified_counts_adj.csv")[,-1]
#nonsevere_infection_multipliers <- read.csv("data/nonsevere_infection_multipliers.csv")
...
age_75_plus <-  read.csv("calibration-results/adj-calibration-1mil-75+ years.csv")[,-1]
...
#mutate(ve_pred = if_else(ve_pred + 0.1 > 1, ve_pred, ve_pred + 0.1))
...
outcome_occurrence <- function(age, inf, time, lambda, perfect_immunity_counter, death_marker) {
  ...
  nonsevere_risk <- severe_risk * 0.3 * nonsevere_multiplier
...
noBoosterSimulation <- function(df){
  ...
  inf <- input$prior_inf
  time_since_last <- input$months_since_last_dose_inf #No Booster
  lambda <- input$lambda
  ...
  write.csv(input, paste0("simulation-results/second-pass-100000-adj/noBooster-", age_info, "wo_multiplier.csv"))
}
...
oneBoosterSimulation <- function(df){
  ...
  write.csv(input, paste0("simulation-results/fifth-pass-1mil-smoothing-6mo/1Booster-", age_info, "-lower.csv"))
}
...
annualBoosterSimulation <- function(df){
  ...
  write.csv(input, paste0("simulation-results/fifth-pass-1mil-smoothing-6mo/annualBooster-", age_info, "-lower.csv"))
}
...
biannualBoosterSimulation <- function(df){
  ...
  write.csv(input, paste0("simulation-results/fifth-pass-1mil-smoothing-6mo/biannualBooster-", age_info, "-lower.csv"))
}
