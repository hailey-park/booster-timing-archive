...
#Load data
hosp_death_age_stratified <- data.frame(age_group = c("immunocompromised"),perc_death = c(0.074723946))
nonsevere_infection_multipliers <- data.frame(age_group = c("immunocompromised"), multiplier = c(16.857))

############################################################################
#MAKE SURE LOADING IN CORRECT WANING DATA FROM CORRECT FOLDER
waning_data_mean <- read.csv("data/ve_waning_predictions_mean_immunocompromised.csv")[,-1] 
waning_data_95UI <- read.csv("data/ve_waning_predictions_95UI_immunocompromised.csv")[,-1]
...
oneBoosterSimulation <- function(df){
  ...
  write.csv(colSums(input[, (7:31)]), "simulation-results/sensitivity/delayed-vaccine-administration/1Booster-immuno-mean.csv")
}
...
annualBoosterSimulation <- function(df){
  ...
  write.csv(colSums(input[, (7:31)]), paste0("simulation-results/sensitivity/delayed-vaccine-administration/annualBooster-immuno-mean.csv"))
}
...
biannualBoosterSimulation <- function(df){
  ...
  write.csv(colSums(input[, (7:31)]), paste0("simulation-results/sensitivity/delayed-vaccine-administration/biannualBooster-immuno-mean.csv"))
}
