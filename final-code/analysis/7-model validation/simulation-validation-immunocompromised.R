...
oneBoosterSimulation <- function(df){
  ...
  print(vaccine_spread_prob)
  averted['vaccine_wave'] <- sample(c(0:3), nrow(averted), prob = vaccine_spread_prob, replace = TRUE)
  ...
  write.csv(colSums(input[, (8:32)]), "simulation-results/random/validation-Booster-immuno-mean.csv")
}
