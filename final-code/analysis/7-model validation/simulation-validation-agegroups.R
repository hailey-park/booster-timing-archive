...
clean_age_matrix <- function(df){
  df %>% dplyr::select(c("individual", "age_group", "prior_inf", "months_since_last_dose_inf", "num_doses","lambda")) %>%
    mutate(baseline_biv_cov = case_when(age_group %in% c("18-49 years", "50-64 years") ~ 0.147,
                                        age_group %in% c("65-74 years", "75+ years") ~ 0.326,
                                        TRUE ~ 0))
}
...
oneBoosterSimulation <- function(df){
  ...
  if (baseline_bivalent_coverage == 0.326) {
    vaccine_spread_prob <- c(0.674, 0.108, 0.109, 0.109)
  }
  ...
  write.csv(colSums(input[, (8:32)]), paste0("simulation-results/random/validation-1Booster-", age_info, "-mean.csv")) ####CHANGE HERE
}

set.seed(88)
clean_df[3:4] %>% lapply(oneBoosterSimulation)
