...
age_75plus <- read.csv("calibration-results/main/adj-calibration-1mil-75+ years-mean.csv")[,-1]
age_75plus <- read.csv("calibration-results/sensitivity/100-seroprevalence/adj-calibration-1mil-50-64 years-mean.csv")[,-1]
...
time_since_75plus <- age_75plus %>% group_by(time_since_last_dose_inf) %>% summarise(total = n()) %>% 
  mutate(time_since_last = as.Date(time_since_last_dose_inf),
         age_group = "75+ years")
